...
// ── Checkout-photo evidence (Phase 1) ───────────────────────────────────────
// A worker's proof-of-work photos, surfaced from their check-out; the admin
// reviews them to judge progress. Images open in a lightbox; PDFs in a new tab.
...
// Collapsible project-level "site evidence" panel on the Tareas tab: present
// workers + their checkout photos for a chosen date, grouped by worker.
const evidenceOpen = ref(false);
const evidenceDate = ref(new Date().toISOString().slice(0, 10));
const evidenceWorkers = ref([]);
const evidenceLoading = ref(false);
async function fetchEvidence() {
    evidenceLoading.value = true;
    try {
        const res = await fetch(`/projects/${props.project.id}/present-workers?date=${evidenceDate.value}`, { headers: { Accept: 'application/json' } });
        const data = await res.json();
        evidenceWorkers.value = data.workers ?? [];
    } catch { evidenceWorkers.value = []; }
    evidenceLoading.value = false;
}
function toggleEvidence() {
    evidenceOpen.value = !evidenceOpen.value;
    if (evidenceOpen.value && evidenceWorkers.value.length === 0) fetchEvidence();
}
...
<template>
    ...
    <AppLayout>
        ...
        <div class="mt-5">
            ...
            <div v-else-if="tab === 'tasks'" class="space-y-4">
                ...
                <template v-else-if="projectTasks">
                    <!-- Site evidence (Phase 1): checkout proof-of-work photos of the
                         workers present on a chosen day, grouped by worker. Read-only
                         reference for judging task progress; collapsed by default. -->
                    <VCard :padded="false">
                        <button type="button" class="flex w-full items-center justify-between px-4 py-3 text-start" @click="toggleEvidence">
                            <span class="flex items-center gap-2 text-[13px] font-semibold text-ink">
                                <AppIcon name="camera" class="h-4 w-4 text-muted" />
                                {{ $t('task_progress.evidence_title') }}
                            </span>
                            <AppIcon name="chevron-right" class="h-4 w-4 text-muted transition-transform" :class="{ 'rotate-90': evidenceOpen }" />
                        </button>
                        <div v-if="evidenceOpen" class="border-t border-line px-4 py-3">
                            <div class="mb-3 flex items-center gap-2">
                                <VDateInput v-model="evidenceDate" class="w-44" @update:model-value="fetchEvidence" />
                                <span class="text-xs text-muted">{{ $t('task_progress.evidence_hint') }}</span>
                            </div>
                            <div v-if="evidenceLoading" class="py-3 text-sm text-muted">{{ $t('common.loading') }}</div>
                            <div v-else-if="evidenceWorkers.length === 0" class="py-3 text-sm text-muted">{{ $t('task_progress.none_present') }}</div>
                            <div v-else class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                                <div v-for="w in evidenceWorkers" :key="w.id" class="rounded-lg border border-line p-3">
                                    <p class="text-sm font-medium text-ink">{{ w.name }}</p>
                                    <p v-if="w.designation" class="text-xs text-muted">{{ w.designation }}</p>
                                    <div v-if="w.checkout_photos && w.checkout_photos.length" class="mt-2 flex flex-wrap gap-1.5">
                                        <button v-for="p in w.checkout_photos" :key="p.which" type="button"
                                            class="h-14 w-14 overflow-hidden rounded-md border border-line bg-surface-sunken transition hover:ring-2 hover:ring-accent"
                                    ...
                                    <p v-else class="mt-2 text-[11px] text-muted">{{ $t('task_progress.no_photos') }}</p>
                                </div>
                            </div>
                        </div>
                    </VCard>
                                ...
                                <tbody>
                                    <template v-for="t in projectTasks.tasks" :key="t.id">
                                        ...
                                        <tr class="border-b border-line hover:bg-surface-hover" :class="{ 'border-b-0': isExpanded(t) }">
                                            ...
                                            <td class="tabular-nums px-3 py-2 text-end">{{ t.completed_quantity }} {{ t.unit }}</td>
                                            <td class="px-3 py-2">
                                        ...
                                        <template v-if="isExpanded(t) && t.children.length">
                                            <template v-for="c in t.children" :key="c.id">
                                                <tr class="border-b border-line bg-surface-sunken/30 hover:bg-surface-hover" :class="{ 'border-b-0': isExpanded(c) }">
                                                    ...
                                                    <td class="tabular-nums px-3 py-2 text-end">{{ c.completed_quantity }} {{ c.unit }}</td>
                                                    <td class="px-3 py-2">
                                                        <div class="flex items-center gap-2">
                                                            <div class="h-2 w-24 overflow-hidden rounded-full bg-surface-sunken">
                                ...
                                <table class="w-full text-sm">
                                    ...
                                    <tbody>
                                        <tr v-for="r in taskReport.planned_vs_actual.rows" :key="r.id" class="border-b border-line last:border-0">
                                            ...
                                            <td class="tabular-nums px-3 py-2 text-end">{{ r.done }} {{ r.unit }}</td>
                                            <td class="tabular-nums px-3 py-2 text-end">{{ r.remaining }} {{ r.unit }}</td>
        ...
        <div v-if="photoLightbox" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4"
