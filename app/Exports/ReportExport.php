...
class ReportExport implements FromArray, WithHeadings, WithTitle
{
    ...
    public function headings(): array
    {
        return match ($this->module) {
            ...
            'profitability' => ['Proyecto / Project', 'Cliente / Client', 'Horas / Hours', 'Ingresos / Revenue', 'Coste MO real / True labour', 'Seg. social / Employer tax', 'Otros empresa / Other company', 'Otros cliente / Other client', 'Beneficio / Profit', 'Margen % / Margin %', 'Gasto operativo / Operational overhead'],
    ...
    public function array(): array
    {
        return match ($this->module) {
            ...
            'profitability' => array_map(fn (array $r): array => [
                $r['project'], $r['client'], $r['hours'], $r['revenue'], $r['coste_mo'],
                $r['employer_tax'] ?? 0, $r['expenses_company'] ?? 0, $r['expenses_client'] ?? 0,
                $r['profit'], $r['margin'] ?? '—', $r['operational_overhead'] ?? 0,
            ], $this->report['rows']),
